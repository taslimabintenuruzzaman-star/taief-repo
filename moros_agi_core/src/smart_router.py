...
class SmartRouter:
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._local = httpx.AsyncClient(timeout=2.0, follow_redirects=True)
    ...
    async def complete(
        ...
        gemini_on = bool(self.settings.secret(self.settings.gemini_api_key))
        steps: list[tuple[str, Any]] = []
        if gemini_on:
            steps.append(("gemini", lambda: self._gemini(messages, task)))
        else:
            steps.append(("ollama", lambda: self._ollama(messages, self.local_model_for(task), task)))
        steps.extend(
            [
        ...
        if not gemini_on:
            steps.insert(2, ("gemini", lambda: self._gemini(messages, task)))
    ...
    async def _gemini(self, messages: list[dict[str, str]], task: TaskKind) -> RouteResult:
        ...
        models = [
            ...
            self.settings.gemini_model_pro,
            "gemini-2.0-flash",
            "gemini-1.5-flash",
            "gemini-1.5-pro",
            "gemini-flash-latest",
        ...
        for model in models:
            ...
            try:
                res = await self._client.post(url, params={"key": key}, json=payload)
                ...
                try:
                    async with httpx.AsyncClient(
                        ...
                        res = await insecure.post(url, params={"key": key}, json=payload)
        ...
        raise ProviderError("gemini", last)
